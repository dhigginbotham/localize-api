...
(function() {
  ...
  localize = function(opts) {
    ...
    if (this.prefix.indexOf("/" === 0 || this.prefix.lastIndexOf("/" === 0))) {
      this.prefix.replace("/", "");
    }
    if (this.customKey === null) {
      this.customKey = this.prefix;
    }
    self = this;
  ...
  localize.prototype.mount = function(app) {
    ...
    if (self.customRoute === null) {
      ...
      app.all("/" + self.prefix + "*", self.request);
    } else {
      app.all("/" + self.prefix + "*", self.request, self.customRoute);
    }
    return this;
  };
